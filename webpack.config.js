...
const pathToSSG = {
  '/posts/webgpu-triangle': { title: 'Triangle rendering - WebGPU' },
  '/posts/webgpu-setup': { title: 'WebGPU setup guide' },
  '/posts/ultimate-popover': { title: 'The ultimate popover - HTML + CSS' },
  '/': { title: 'Home' },
}
...
module.exports = {
            ...
            use: [
              {
                ...
                options: {
                  postcssOptions: {
                    plugins: [
                      ['cssnano'],
                      // preset: advanced got us almost no optimization, and
                      // css counter is impossible to use then, it changes the name of the coutner
                    ],
                  },
                },
              },
  ...
  output: {
    ...
    clean: true,
    publicPath: '/', // this is needed for HTMLWebpackPlugin to use absoltue path, otherwise all paths are relative and start with current URL, not to "/"
  },
  optimization: {
    ...
    new PuppeteerPrerenderPlugin({
      ...
      postProcess: (result) => {
        result.html = result.html
          ...
          .replace('<title></title>', '<title>' + pathToSSG[result.route]?.title + '</title>')
          .replace(/mounted="true"/g, 'mounted="true" hydration="true"')
      },
